...
#games = pd.read_csv("data/influence_prep2.csv")
games = pd.read_csv("influence_prep2.csv")
...
def def_i(play_df, n, i, P):
    def_df = play_df[(play_df.frame == i) & (play_df.side == "DEF")]
...
def off_i(play_df, n, i, P):
    off_df = play_df[(play_df.frame == i) & (play_df.side == "OFF")]
...
def influence_areas(play_df, i):
    ...
    for n in range(11):
        Z1_total += def_i(play_df, n, i, P)

    Z2_total = np.zeros(P.shape[0])
    for n in range(10):
        Z2_total += off_i(play_df, n, i, P)
...
def current_mat(mat0):
    mat = np.where(
        ...
        np.where(
            (mat0 >= 0.8) & (mat0 <= 0.89),
            2,  # when... then
            np.where((mat0 >= 0.9) & (mat0 <= 1), 3, 0),  # when... then
        ),
    )
...
def calc_control_and_trajectory(x, dw, config, goal, ob, ia):
    ...
    ia_costs = config.zone_cost_gain * calc_influence_area_cost(
        trajectories, config, ia
    )
...
def dwa(gameId, playId, i):
    #print(f"on frame ID {i}")
    play = games[(games.gameId == gameId) & (games.playId == playId)]
    play_df = play[play.frame == i]
    def_df = play_df[(play_df.side == "OFF")]
    off_df = play_df[(play_df.side == "DEF")]
    # initial state [x(yds), y(yds), yaw(rad), v(yds/s), omega(rad/s)]
    # orientation used could apply carrier direction here; car_dir
    car_dir = abs(play_df.iloc[0]["car_dir"])
    car_s = play_df.iloc[0]["car_s"]
    x = np.array(
        [
            play_df.iloc[0]["car_x"],
            play_df.iloc[0]["car_y"],
            car_dir,
            car_s,
            0.125 * math.pi,
        ]
    ...
    goal = np.array([110, play_df.iloc[0]["car_y"]])
    ...
    projection = [
        play_df.iloc[0]["gameId"],
        play_df.iloc[0]["playId"],
        i,
        play_df.iloc[0]["car_x"],
        play_df.iloc[0]["car_y"],
        trajectory[1, 0],
...
# Turn this into a parallel processing format
gameId = 2022091800
playId = 2801

num_cpus = os.cpu_count()
if num_cpus == 1:
    logging.warning("This computer reports to only have one CPU core.")
elif num_cpus == 2:
    logging.warning(
        "This computer reports to only have two CPU cores."
        + "\nA 4+ core computer running this script should "
        + "be able to run this script faster than what this computer can."
    )
elif num_cpus > 2:
    num_cpus-=1
...
play = games[(games.gameId == gameId) & (games.playId == playId)]
frame_numbers = play["frame"].unique()

# results = []

# for frame in tqdm(frame_numbers):
#     result = dwa(gameId, playId, frame)
#     results.append(result)

results = pool.map(dwa,(gameId, playId, frame_numbers))
